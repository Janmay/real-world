...
const callApi = (endpoint: string, resSchema: Schema) => {
        ...
        .then(response => 
            response.json().then<object>((json) => {
                ...
                return Object.assign({},
                    normalize(camelizedJson, resSchema),
                    { nextPageUrl }
                )
            })
        )
...
export default (store: MiddlewareAPI<Dispatch, StoreState>) => (next: Dispatch) => (action: AnyAction) => {
    ...
    return callApi(endpoint, apiSchema).then(
